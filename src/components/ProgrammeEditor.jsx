        ...
        {programme.blocks.map((block, blockIdx) => (
          <div key={block.id} style={s.block}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 8 }}>
              ...
              />
              <button onClick={() => removeBlock(block.id)} style={s.removeBtn} aria-label="Remove block">&times;</button>
            </div>
              ...
              {block.activities.map((activity, activityIdx) => (
                <div key={activity.id} style={s.activityRow}>
                  ...
                  <input
                    ...
                    style={{ ...s.input, flex: 1 }}
                  />
                  <button onClick={() => removeActivity(block.id, activity.id)} style={s.removeBtn} aria-label="Remove exercise">&times;</button>
                  {errors[`activity-${activity.id}-name`] && <span style={s.error}>{errors[`activity-${activity.id}-name`]}</span>}
                </div>
              ))}
              <button onClick={() => addActivity(block.id)} style={s.dashedBtn}>+ Add Exercise</button>
            </div>
          </div>
        ))}
        <button onClick={addBlock} style={s.dashedBtn}>+ Add Block</button>
      </div>

      <div style={{ display: 'flex', gap: 12, marginTop: 32 }}>
        <button onClick={onCancel} style={s.secondaryBtn} disabled={saving}>Cancel</button>
        <button onClick={handleSave} style={s.primaryBtn} disabled={saving}>{saving ? 'Saving…' : 'Save'}</button>
      </div>
    </div>
  )
}
...
const s = {
  ...
  removeBtn: {
    ...
    border: '1px solid var(--color-error-border)', borderRadius: 'var(--radius-md)', cursor: 'pointer', fontSize: 16, lineHeight: 1,
  },
  dashedBtn: {
